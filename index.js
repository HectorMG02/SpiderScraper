 ...
 async function spiderScrapper(){
    ...
    await fs.readFile('multiverse.txt', 'utf8', async (err, data) => {
        ...
        if (data.length > 0) {
            ...
            const film_available = hrefs.filter(href => href.includes(film_to_search)).length ? true : false;
            ...
            const href_to_search = hrefs.filter(href => href.toLowerCase().includes(film_to_search))[0];
            ...
            console.log('Las 2 primeras horas disponibles para la pelicula son:', available_times, 'vamos a ver cuanta gente hay en la primera y en la segunda sesion');
...
async function check_sites(sessionId, page, film_day, hour, film_url) {
    ...
    await page.click(`.${film_day} > .item-session > .show-session-sessions > .btn_sesion:nth-child(${sessionId})`);
    ...
    available_sites = available_sites.concat(await page.$$eval('img[data-seat="002"]', anchors => {
        return anchors.map(anchor => {
            return {
                row: Number(anchor.dataset.row),
                column: Number(anchor.alt)
    ...
    let sites_between_12_and_9 = available_sites.filter(site => site.row <= 12 && site.row >= 9 && site.column > 4 && site.column < 15);
